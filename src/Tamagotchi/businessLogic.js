...
function getNextStateAttributes (tamagotchi) {
  ...
  function getNextAwakeTick () {
    if(tamagotchi.state.awakeTick === tamagotchi.timeTick || tamagotchi.state.tiredness === 0) {
      return null;
    }
  ...
  function getNextHungryness () {
    return Math.min(tamagotchi.state.hungryness + Tamagotchi.HUNGRY_INCREMENT, 100)
  }

  function getNextTiredness () {
    if( tamagotchi.isSleeping ) {
      return Math.max(tamagotchi.state.tiredness - Tamagotchi.TIRED_INCREMENT, 0)
    } else {
      return Math.min(tamagotchi.state.tiredness + Tamagotchi.TIRED_INCREMENT, 100);
    }
  }
}
